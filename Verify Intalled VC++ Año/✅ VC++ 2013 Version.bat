...
title Mr. DragonStar VC++ 2013
cls

echo Obteniendo informacion de Visual C++ 2013...
...
:: Buscar versiones x86 de VC++ 2013
for /f "tokens=*" %%a in ('reg query "HKLM\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall" /s /f "Microsoft Visual C++ 2013" 2^>nul') do (
    set "key=%%a"
    for /f "tokens=1,2,*" %%b in ('reg query "!key!" /v DisplayName 2^>nul ^| find "REG_SZ"') do (
        set "name=%%d"
        :: Filtrar solo entradas x86
        echo !name! | find "x86" >nul && (
...
:: Buscar versiones x64 de VC++ 2013 (solo en sistemas de 64 bits)
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
    set "prev_name="
    for /f "tokens=*" %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall" /s /f "Microsoft Visual C++ 2013" 2^>nul') do (
        set "key=%%a"
        for /f "tokens=1,2,*" %%b in ('reg query "!key!" /v DisplayName 2^>nul ^| find "REG_SZ"') do (
            set "name=%%d"
            :: Filtrar solo entradas x64
            echo !name! | find "x64" >nul && (
                for /f "tokens=1,2,*" %%e in ('reg query "!key!" /v DisplayVersion 2^>nul ^| find "REG_SZ"') do set "ver=%%g"
                for /f "tokens=1,2,*" %%e in ('reg query "!key!" /v Publisher 2^>nul ^| find "REG_SZ"') do set "pub=%%g"
                for /f "tokens=1,2,*" %%e in ('reg query "!key!" /v InstallDate 2^>nul ^| find "REG_SZ"') do set "date=%%g"
...
if !x86_found! equ 0 (
    echo Microsoft Visual C++ 2013 x86 Redistributable         NO ENCONTRADO
)
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" if !x64_found! equ 0 (
    echo Microsoft Visual C++ 2013 x64 Redistributable         NO ENCONTRADO
)
