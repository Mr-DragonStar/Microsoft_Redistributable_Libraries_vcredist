...
:: Buscar versiones x86 de VC++ 2012
for /f "tokens=*" %%a in ('reg query "HKLM\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall" /s /f "Microsoft Visual C++ 2012" 2^>nul') do (
    set "key=%%a"
...
:: Buscar versiones x64 de VC++ 2012 (solo en sistemas de 64 bits)
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
    set "prev_name="
    for /f "tokens=*" %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall" /s /f "Microsoft Visual C++ 2012" 2^>nul') do (
        set "key=%%a"
        for /f "tokens=1,2,*" %%b in ('reg query "!key!" /v DisplayName 2^>nul ^| find "REG_SZ"') do (
            set "name=%%d"
