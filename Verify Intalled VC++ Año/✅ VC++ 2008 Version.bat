...
title Mr. DragonStar VC++ 2008
cls

echo Obteniendo informacion de Visual C++ 2008...
...
:: Buscar versiones x86 de VC++ 2008
for /f "tokens=*" %%a in ('reg query "HKLM\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall" /s /f "Microsoft Visual C++ 2008" 2^>nul') do (
    set "key=%%a"
...
:: Buscar versiones x64 de VC++ 2008 (solo en sistemas de 64 bits)
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
    set "prev_name="
    for /f "tokens=*" %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall" /s /f "Microsoft Visual C++ 2008" 2^>nul') do (
        set "key=%%a"
        for /f "tokens=1,2,*" %%b in ('reg query "!key!" /v DisplayName 2^>nul ^| find "REG_SZ"') do (
            set "name=%%d"
...
if !x86_found! equ 0 (
    echo Microsoft Visual C++ 2008 x86 Redistributable         NO ENCONTRADO
)
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" if !x64_found! equ 0 (
    echo Microsoft Visual C++ 2008 x64 Redistributable         NO ENCONTRADO
)
